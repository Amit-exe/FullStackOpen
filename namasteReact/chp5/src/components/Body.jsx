...
const url_res =
  "https://www.swiggy.com/dapi/restaurants/list/v5?lat=19.1760819&lng=73.02288949999999&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING";
const Body = () => {
  const [topRated, settopRated] = useState(false);
  const [toSearch, setTosearch] = useState("");
  const [showRes, setShowRes] = useState([]);
  const fr = showRes.filter((r) =>
    r.info.name.toLowerCase().includes(toSearch.toLowerCase()),
  );

  const topRatedRes = () => {
    console.log("top");
    console.log(fr[0].info.avgRating);
  ...
  if (!showRes) return <h1>Loading...</h1>;
